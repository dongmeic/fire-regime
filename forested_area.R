...
png("area_burned_ann.png", width=12, height=6, units="in", res=300)
par(xpd=FALSE,mfrow=c(1,2),mar=c(0.5,0.5,2.5,0.5))
quick.map2(pro.ffdf.tr,"BAsum_ann",expression('Area burned (ha·yr'^-1*')'), "Mean annual area burned from MODIS", "Reds", "quantileCuts")
quick.map2(pro.ffdf.tr,"Sum_ann",expression('Area burned (ha·yr'^-1*')'), "Mean annual area burned from CFSY", "Reds", "quantileCuts")
dev.off()
...
title("Mean annual area burned from MODIS",cex.main=1.5)
choro.legend(-870000,2900000, shades, bty='n', cex=1, title=expression('Area burned (ha·yr'^-1*')'))
add.northarrow()
add.scale()
choropleth(pro.ffdf.tr, pro.ffdf.tr$Sum_ann, shades)
title("Mean annual area burned from CFSY",cex.main=1.5)
choro.legend(-870000,2900000, shades, bty='n', cex=1, title=expression('Area burned (ha·yr'^-1*')'))
add.northarrow()
add.scale()
dev.off()
