...
pro.shp <- readShapePoly(pro.shpfile, proj4string = CRS("+proj=aea +lat_1=25 +lat_2=47 +lat_0=30 +lon_0=105 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))
# color <- 'YlOrRd'
...
bound <- list("sp.polygons", pro.shp, col="grey", lwd=0.8, first = FALSE)
...
rgtypes <-c('LHLHHf', 'LLLLLfc', 'LHMHHg', 'HLHLHfs', 'HLMLHc', 'LLLHLg')
rstack <- stack()
...
for (i in seq(1, 12, by=4)){
  ...
  p1 <- levelplot(layer1, at=getJenksBreaks(na.omit(getValues(layer1)), 6), scales=list(draw=FALSE), margin=F, par.settings=mapTheme, main=names0[i])+latticeExtra::layer(sp.polygons(pro.shp, lwd=0.8, col='gray'))
  p2 <- levelplot(layer2, at=getJenksBreaks(na.omit(getValues(layer2)), 6), scales=list(draw=FALSE), margin=F, par.settings=mapTheme, main=names0[i+1])+latticeExtra::layer(sp.polygons(pro.shp, lwd=0.8, col='gray'))
  p3 <- levelplot(layer3, at=getJenksBreaks(na.omit(getValues(layer3)), 6), scales=list(draw=FALSE), margin=F, par.settings=mapTheme, main=names0[i+2])+latticeExtra::layer(sp.polygons(pro.shp, lwd=0.8, col='gray'))
  p4 <- levelplot(layer4, at=getJenksBreaks(na.omit(getValues(layer4)), 6), scales=list(draw=FALSE), margin=F, par.settings=mapTheme, main=names0[i+3])+latticeExtra::layer(sp.polygons(pro.shp, lwd=0.8, col='gray'))
...
regimes <- stack(paste(inpath, "regime_08-May-2017.tif", sep=""))
...
mycolors1 <- c('#984ea3', '#ffff33', '#4daf4a', '#ff7f00', '#e41a1c', '#377eb8')
mycolors2 <- c('#e41a1c', '#377eb8', '#4daf4a', '#ff7f00', '#984ea3', '#ffff33')
mykey <- list(text=list(lab=c("Fire regime 1", "Fire regime 2", 
              ...
              rectangles=list(col=mycolors2), space="inside", width=0.2, columns=3)
# gamma = 2.2
...
#regimes.g <- rgb2gray("regime_08-May-2017.tif")
regimes <- raster(paste(inpath,"regime_08-May-2017.tif", sep=""))
...
p$legend$inside$x <- 0.1
p$legend$inside$y <- 0.98
plot(p)
dev.off()
...
p1 <- spplot(regime1, draw = T, col.regions=mycolors1[1], colorkey=FALSE, main=paste("Fire regime 1:", rgtypes[1]),
            scales=list(draw=F), sp.layout=list(bound))
plot(p1)

regime2 <- read.raster("regime_2.tif")
p2 <- spplot(regime2, draw = T, col.regions=mycolors1[2], colorkey=FALSE, main=paste("Fire regime 2:", rgtypes[2]),
             scales=list(draw=F), sp.layout=list(bound))
plot(p2)

regime3 <- read.raster("regime_3.tif")
p3 <- spplot(regime3, draw = T, col.regions=mycolors1[3], colorkey=FALSE, main=paste("Fire regime 3:", rgtypes[3]),
             scales=list(draw=F), sp.layout=list(bound))
plot(p3)

regime4 <- read.raster("regime_4.tif")
p4 <- spplot(regime4, draw = T, col.regions=mycolors1[4], colorkey=FALSE, main=paste("Fire regime 4:", rgtypes[4]),
             scales=list(draw=F), sp.layout=list(bound))
plot(p4)

regime5 <- read.raster("regime_5.tif")
p5 <- spplot(regime5, draw = T, col.regions=mycolors1[5], colorkey=FALSE, main=paste("Fire regime 5:", rgtypes[5]),
             scales=list(draw=F), sp.layout=list(bound))
plot(p5)

regime6 <- read.raster("regime_6.tif")
p6 <- spplot(regime6, draw = T, col.regions=mycolors1[6], colorkey=FALSE, main=paste("Fire regime 6:", rgtypes[6]),
             scales=list(draw=F), sp.layout=list(bound))
plot(p6)
...
proj4string(mtains) <- ch.aea
LC.China <- raster("/Volumes/dongmeic/fire/output/revision/LC_China/LC_China_2013.tif")
