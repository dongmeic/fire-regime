...
for idx = 1:length(years)
	...
	for jdx = 1:length(months)
		v =months(jdx);
		% read data of year y and month v
		map = geotiffread([basedir 'BA_China_' num2str(y) '-' num2str(v) '.tif']);
		fires= zeros(m, n);
		
		%% Find all the fires points in the map corresponding to year y and
		%% month v
		% fires variable will be different from zero where a fire is found,
		% furthermore, the fire locations will be grouped in connected
		% regions:
		% the value of fires(ii,jj) corresponds to the connected region
		% index
        %% Find all the fires points.
        [I, J, dates] = find(map .* ROI);
        len = length(I);
        %% Flood-Fill algorithm
        fireID = 0;
        for k = 1:len
          [first_fire, kdx] = min(dates);
          i = I(kdx);
          j = J(kdx);
          if fires(i, j) == 0
            fireID = fireID + 1;
		...
		for jj = 1:nn
			for ii = 1:mm
				i = xIndex(:, ii);
				j = yIndex(:, jj);
				% fires_grid: matrix containing local values of the fires
				% variable, at the i,j position of the grid previously
				% defined with xIndex, yIndex
				fires_grid = fires(i(1):i(2), j(1):j(2));
				% detecting fire presence: a will be empty if fire is not
				% detected, while it will be a vector containing the list
				% of elements different from 0 found (i.e. it can contain
				% numbers from 1 to the total number of connected regions)
				a = fires_grid(fires_grid > 0);
				% b contains the list of connected region indexes found in
				% this part of the grid
				b = unique(a);
				% c = number of connected regions indexes found here
				c = numel(b);
                d = zeros(1, c);
                for e = 1:c
                    % counting the number of pixels for each connected
                    % region found here
                    d(e) = sum(a == b(e));
                end
                %jdx: month index
				...
				% list of fire areas at point ii jj of the grid: the list
				% takes into account of one value (if any fire) per each month
...
for jj = 1:nn
	for ii = 1:mm
		...
		if any(fireshist{ii, jj})
			...
			if Gini(ii, jj) == 0
				number_of_sum_z_equal_to_1 = number_of_sum_z_equal_to_1 + 1;
			end
		end
	end
end
...
tic
manof = zeros(mm, nn); % mean annual number of fires
anofstd = zeros(mm, nn); % standard deviation of annual number of fires
...
for jj = 1:nn
	for ii = 1:mm
		if manof(ii, jj) ~= 0 & manof(ii, jj) ~= -1
...
fireshist_greater_than_0 = zeros(mm, nn);
for jj = 1:nn
	for ii = 1:mm
		if ~isempty( fireshist{ii, jj} )
			fireshist_greater_than_0( ii, jj ) = 1;
		end
	end
end
disp( ['Gini index is 0 in ' num2str(number_of_sum_z_equal_to_1) ' places where you do have fire measurements!'] )
disp( ['Total number of places where Gini is different from 0: ' num2str( sum(Gini(:)~=0) ) ] )
disp( ['Total number of places where number_of_fires_y over years is different from 0: ' num2str( sum(sum( any( number_of_fires_y , 3 ) )) )])
disp( ['Total number of places where number_of_months_fire_y over years is different from 0: ' num2str( sum(sum( any( number_of_months_fire_y , 3 ) )) )])
disp( ['Total number of places where manof is different from 0: ' num2str( sum(sum( manof > 0 )) )])
disp( ['Total number of places where anofstd is different from 0: ' num2str( sum(sum( anofstd > 0 )) )])
disp( 'anofstd is different from 0 in a lower number of places than manof, but this is correct: std is 0 where you have only one measurent different from 0' )
disp( ['Total number of places where fireshist is non-empty: ' num2str( sum( fireshist_greater_than_0(:) ))])
